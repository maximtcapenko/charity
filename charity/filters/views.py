...
from .forms import CreateFilterForm, CreateFilterExpressionForm
...
def add_filter(request):
    ...
    return_url = reverse('filters:get_list')
    content_type = get_object_or_404(ContentType, model='ward')
...
def get_list(request):
    fund = request.user.volunteer_profile.fund
    paginator = Paginator(Filter.objects.filter(fund=fund), DEFAULT_PAGE_SIZE)
